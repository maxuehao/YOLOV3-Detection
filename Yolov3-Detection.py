			...
			for k in range(3):
				...
				if score > confidence:
					...
					if length == feature_map_size:
						w = (((BIAS_W[k+6]) * math.exp(anchors_box[2]))/INPUT_SIZE)*iw
						h = (((BIAS_H[k+6]) * math.exp(anchors_box[3]))/INPUT_SIZE)*ih
					...
					elif length == feature_map_size*4: 
...
if __name__ == "__main__":
	...
	while True:
		...
		#绘制检测框
		for i in output_box:
			cv2.rectangle(test_img, (i[0], i[1]), (i[2], i[3]), (0, 255, 0), 3)
